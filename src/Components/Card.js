...
function Card(){
    ...
    const [americareal, setAmericareal] = useState(false);
    ...
    */
    ...
    const fetchUser = async () => {
      ...
      const apiCallJ = await fetch (`https://stats.iosoccer-sa.bid/api/player/${playerID}/t6`);
      ...
      const apiCallI = await fetch (`https://stats.iosoccer-sa.bid/api/player/${playerID}/copaamerica`);
      ...
      const totaltimet6 = userJ[0] != undefined ? ( userJ[0].secondsplayed/60/90 > userJ[0].matches ? userJ[0].matches : userJ[0].secondsplayed/60/90 ) : 0;
      ...
      console.log("SAVE PERCENT VIEJO / SAVE NUEVO / SAVE MULTIPLICADO: "+(doSomethingWithInput(SAVEPERCENT))+" / "+(SAVEPERCENT/actualtime)+" / "+SAVEPERCENT);
      ...
      const CONCEDED = Math.abs((1-(user[0].goalsconceded/SAVES))*130);
      const SACRIFICIO = Math.abs((user[0].distancecovered * (100-(user[0].possession/(actualtime/user[0].matches)))) / 7000);
    ...
    */
    ...
    const fetcht1 = async () => {
      ...
      let POSS = Math.round((usert1[0].possession/(totaltime/usert1[0].matches) * 7.76));
      ...
      let SAVEPERCENT = Math.round((usert1[0].savescaught/usert1[0].saves)*110);
    ...
    /*const fetchamerica = async () => {
      ...
      if(usert1[0] && usert1[0].matches > 3)
      {
        ...
        let CONCEDED = Math.abs(((usert1[0].goalsconceded-usert1[0].saves)/SAVES*100)*1.5);
        CONCEDED = (Math.round(doSomethingWithInput(CONCEDED)));
              ...
              <div className="top-info">
                ...
                <div>
                  <ul className="versions-list">
                    ...
                    <div>
                    {t6real ? <button className="abutton" onClick={r => setTID(String("t6")) && setActualovr(t6)}>
                      <li className="versions-list-el">
